...
void main() {
    ...
    test('Non-Base83 boundary ASCII characters are strictly rejected', () {
      ...
      for (final char in forbidden) {
        ...
        expect(
          ...
          isFalse,
          reason:
              'Character "$char" (code ${char.codeUnitAt(0)}) must be rejected',
        );
      }
    });

    test(
      'Unicode characters with code unit >= 256 are safely handled and rejected',
      () {
  ...
  group(
    'BlurHashDecoder Adversarial: Component Flags & Boundary Configurations',
    () {
      ...
      test('1x9 and 9x1 asymmetric component counts', () {
        ...
        expect(
          BlurHashDecoder.decodeToBmp(hash9x1, width: 16, height: 16),
          isNotNull,
        );
        ...
        expect(
          BlurHashDecoder.decodeToBmp(hash1x9, width: 16, height: 16),
          isNotNull,
        );
      });
    },
  ...
  group('BlurHashDecoder Adversarial: Punch & Mathematical Edge Cases', () {
    ...
    test(
      'Handles negative, zero, and extreme punch values without overflow or crash',
      () {
        final punches = [
          -1000.0,
          -10.0,
  ...
  group('BlurHashDecoder Adversarial: Dimension Boundary Stress', () {
    ...
    test('Extreme aspect ratio dimensions (1x256 and 256x1)', () {
      final bmpTall = BlurHashDecoder.decodeToBmp(
        sampleBlurHash,
        width: 1,
        height: 256,
      );
      ...
      final bmpWide = BlurHashDecoder.decodeToBmp(
        sampleBlurHash,
        width: 256,
        height: 1,
      );
    ...
    test('Negative, zero, and degenerate dimensions return null', () {
      expect(
        BlurHashDecoder.decodeToBmp(sampleBlurHash, width: 0, height: 0),
        isNull,
      );
      expect(
        BlurHashDecoder.decodeToBmp(sampleBlurHash, width: -1, height: 32),
        isNull,
      );
      expect(
        BlurHashDecoder.decodeToBmp(sampleBlurHash, width: 32, height: -1),
        isNull,
      );
      expect(
        BlurHashDecoder.decodeToBmp(sampleBlurHash, width: -100, height: -100),
        isNull,
      );

      expect(
        BlurHashDecoder.decodeRgba(sampleBlurHash, width: 0, height: 0),
        isNull,
      );
      expect(
        BlurHashDecoder.decodeRgba(sampleBlurHash, width: -5, height: 10),
        isNull,
      );
    });
  });
  ...
  group('BlurHashDecoder Adversarial: BMP Header & Color Accuracy Oracle', () {
    test(
      'Solid black BlurHash (000000) decodes to pure black RGB(0,0,0,255)',
      () {
    ...
    test(
      'Solid white BlurHash (00TSUA) decodes to pure white RGB(255,255,255,255)',
      () {
    ...
    test(
      'BMP Header Oracle checks all 54 bytes for compliance with Windows DIB standard',
      () {
        const width = 24;
        const height = 18;
        ...
        // BITMAPFILEHEADER
        expect(
          String.fromCharCode(bmp[0]) + String.fromCharCode(bmp[1]),
          equals('BM'),
        );
  ...
  group(
    'BlurHashDecoder Adversarial: LRU Cache Eviction & Boundary Behavior',
    () {
      test(
        'LRU cache strictly caps at 500 entries across 1,200 unique insertions',
        () {
          expect(BlurHashDecoder.cacheSize, equals(0));

          for (int i = 0; i < 1200; i++) {
            BlurHashDecoder.decodeToBmp('000000', punch: 1.0 + (i * 0.0001));
            expect(
              BlurHashDecoder.cacheSize <= 500,
              isTrue,
              reason:
                  'Cache size must never exceed 500, current: ${BlurHashDecoder.cacheSize}',
            );
          }
      ...
      test(
        'LRU cache preserves most recently accessed items and evicts oldest',
        () {
          // 1. Insert item 0
          ...
          final item0Promoted = BlurHashDecoder.decodeToBmp(
            '000000',
            punch: 0.0,
          );
