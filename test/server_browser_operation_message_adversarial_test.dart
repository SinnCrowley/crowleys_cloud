...
void main() {
  ...
  group(
    'ADVERSARIAL: createFolderAtPath operationMessage Lifecycle & Localization',
    () {
      test(
        'Empty and whitespace-only folder names produce validation error without network call',
        () async {
          var networkCalls = 0;
      ...
      test(
        'Success retains localized message across reload in both EN and RU for root and nested paths',
        () async {
      ...
      test(
        'HTTP error responses retain localized failure code in EN and RU',
        () async {
          ...
          for (final code in statusCodesToTest) {
            ...
            // RU test
            await controller.createFolderAtPath(
              'nested',
              'FolderFailRu',
              l10nRu,
            );
      ...
      test(
        'Listeners receive notification with non-null operationMessage upon folder creation',
        () async {
  ...
  group(
    'ADVERSARIAL: moveSelectedToFolder operationMessage Lifecycle & Localization',
    () {
      test(
        'Empty selection does nothing and leaves operationMessage null',
        () async {
      ...
      test(
        'Multiple items (5 items) move success in English and Russian',
        () async {
      ...
      test(
        'All items fail move produces movedNItemsFailedM(0, N) in EN and RU',
        () async {
      ...
      test(
        'Partial failure (3 moved, 2 failed) retains correct count in EN and RU',
        () async {
          final items = List.generate(
            5,
      ...
      test(
        'Moving folder into its own subdirectory fails without network call and updates failure count',
        () async {
          final dirItem = makeItem(
            name: 'parent_folder',
            path: 'parent_folder',
            isDir: true,
          );
  ...
  group('ADVERSARIAL: renameItem operationMessage Lifecycle & Localization', () {
    test(
      'Rename with whitespace-only or identical name returns false and leaves previous operationMessage unchanged',
      () async {
        ...
        // Identical name
        final resSame = await controller.renameItem(
          item,
          'original.txt',
          l10nEn,
        );
        ...
        final resSamePadded = await controller.renameItem(
          item,
          '  original.txt  ',
          l10nRu,
        );
    ...
    test(
      'Success retains localized renamedOldToNew message in EN and RU across reload',
      () async {
        ...
        final controller = await createMockController(
          handler: (req) async {
            if (req.url.path == '/api/files/move' && req.method == 'POST') {
              movedRequests.add(
                Map<String, String>.from(req.url.queryParameters),
              );
              return http.Response('{"ok":true}', 200);
            }
            return http.Response(jsonEncode({'entries': []}), 200);
    ...
    test(
      'Nested file rename preserves parent directory path in dest query parameter',
      () async {
        ...
        final ok = await controller.renameItem(
          item,
          'renamed_child.doc',
          l10nEn,
        );
    ...
    test(
      'Rename HTTP errors (400, 403, 404, 409, 500) produce localized failure message in EN and RU',
      () async {
        ...
        for (final code in errorCodes) {
          ...
          // EN
          final okEn = await controller.renameItem(
            item,
            'new_conflict.txt',
            l10nEn,
          );
          ...
          // RU
          final okRu = await controller.renameItem(
            item,
            'новое_имя.txt',
            l10nRu,
          );
  ...
  group(
    'ADVERSARIAL: Sequencing, Lifecycle Invalidation, and Interleaved Operations',
    () {
      test(
        'Subsequent explicit reload clears operationMessage to null',
        () async {
      ...
      test(
        'Chained sequence: Create Folder -> Rename Item -> Move Selected without explicit reload between calls',
        () async {
          ...
          expect(
            controller.operationMessage,
            l10nEn.renamedOldToNew('test.txt', 'test_v2.txt'),
          );
  ...
  group('ADVERSARIAL: Operation Message Listeners and Notifications', () {
    test(
      'createFolderAtPath notifies listeners and exposes operationMessage in Russian and English',
      () async {
        final controller = await createMockController(
          handler: (req) async {
    ...
    test(
      'renameItem notifies listeners and exposes localized operationMessage',
      () async {
