...
void main() {
  ...
  group('Challenger 2 M3 Protobuf Wire Format & BlurHash Empirical Suite', () {
    ...
    test('1. Wire Format Exact Tag 10 Binary Serialization and Dart Deserialization', () {
      ...
      expect(tagIndex, isNonNegative, reason: 'Field tag 10 wire key 0x52 must exist in serialized bytes');

      final stringLength = buffer[tagIndex + 1];
      expect(stringLength, sampleBlurHash.length);

      final extractedStr = utf8.decode(buffer.sublist(tagIndex + 2, tagIndex + 2 + stringLength));
    ...
    test('2. Deserialization of Large Directory with Mixed Non-Image and Image Entries', () {
      ...
      for (var i = 0; i < totalEntries; i++) {
        ...
        entries.add(
          DirEntry(
            name: isFolder
                ? 'folder_$i'
                : (isPhoto ? 'photo_$i.jpg' : (isDocument ? 'doc_$i.pdf' : 'track_$i.mp3')),
            path: '/items/$i',
            isDir: isFolder,
            size: isFolder ? Int64(0) : Int64(1000 + i * 100),
            modifiedAt: Int64(1725134000 + i),
            type: isFolder ? 'folder' : (isPhoto ? 'photo' : (isDocument ? 'document' : 'audio')),
            mimeType: isFolder
                ? 'inode/directory'
                : (isPhoto ? 'image/jpeg' : (isDocument ? 'application/pdf' : 'audio/mpeg')),
      ...
      debugPrint('2,000 entries Protobuf wire size: ${wireBytes.length} bytes, '
      ...
      for (var i = 0; i < totalEntries; i++) {
        ...
        if (i % 5 >= 3) {
          ...
          expect(item.blurhash, isNull, reason: 'Non-image item $i must have blurhash == null');
          if (item.isDir) {
    ...
    test('4. JSON API Directory Serialization, Deserialization and Special Type Handling', () {
      final testCases = [
        {
          'rawJson': {
            'name': 'image1.png',
    ...
    testWidgets('5. UI Rendering Stability with Mixed Null, Empty, and Valid BlurHash Items',
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  trailing: item.blurhash != null
                      ...
                      : const Icon(Icons.insert_drive_file, key: Key('no_blurhash_icon')),
                );
              },
            ),
          ),
    ...
    test('6. ServerBrowserController End-to-End Protobuf & Filter by Category with BlurHash',
        () async {
      ...
      final mockClient = MockClient((request) async {
        ...
        if (path.endsWith('/api/dir') || path == '/api/dir') {
          ...
          if (typeFilter != null && typeFilter.isNotEmpty && typeFilter != 'all') {
            returnedEntries = returnedEntries.where((e) => e['type'] == typeFilter).toList();
          }
        ...
        if (path.endsWith('/api/account/stats') || path == '/api/account/stats') {
      ...
      final photo = controller.files.firstWhere((f) => f.name == 'vacation.jpg');
      expect(photo.blurhash, equals(sampleBlurHash));
      expect(photo.thumbnailUrl, equals('/api/thumb?path=vacation.jpg&s=256'));

      final audio = controller.files.firstWhere((f) => f.name == 'music.flac');
