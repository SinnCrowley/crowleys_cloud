...
void main() {
  group('UrlUtils', () {
    test('buildEndpoint injects http scheme when missing and resolves relative path', () {
      final uri = UrlUtils.buildEndpoint('localhost:8080', 'dir');
      expect(uri.toString(), 'http://localhost:8080/dir');
    });

    test('buildEndpoint preserves existing https scheme and normalizes trailing slash', () {
      final uri = UrlUtils.buildEndpoint('https://example.com/sub/', '/files/download');
      expect(uri.toString(), 'https://example.com/sub/files/download');
    });

    test('buildApiEndpoint adds /api prefix when missing', () {
      final uri = UrlUtils.buildApiEndpoint('http://localhost:8080', '/dir', {'scope': 'private'});
      expect(uri.toString(), 'http://localhost:8080/api/dir?scope=private');
    });

    test('buildApiEndpoint does not duplicate /api prefix when already present', () {
      final uri = UrlUtils.buildApiEndpoint('http://localhost:8080/api', '/dir');
    ...
    test('normalizeBaseUrl removes trailing slashes', () {
      final normalized = UrlUtils.normalizeBaseUrl('http://example.com:3000///');
