...
class _MockSessionCheckAuthGateway extends _ControllableAuthGateway {
  ...
  Future<SessionCheckResult> checkSession({
    ...
    return SessionCheckResult(sessionStatus, message: sessionCheckMessage);
  }
}
...
void main() {
  ...
  group('CHALLENGER STRESS: M4 Upload & Error Localization Parity', () {
    test(
      'Upload error keys handle boundary conditions, unicode, and special chars in EN & RU',
      () {
    ...
    test(
      'Batch upload summary formatting and count pluralization in EN & RU',
      () {
        final en = lookupAppLocalizations(const Locale('en'));
        final ru = lookupAppLocalizations(const Locale('ru'));
    ...
    test(
      'Disconnected error detector handles case variations, localized Russian, and edge phrases',
      () {
        final en = lookupAppLocalizations(const Locale('en'));
        final ru = lookupAppLocalizations(const Locale('ru'));
        ...
        expect(isDisconnected('Server Disconnected'), isTrue);
        expect(
          isDisconnected('Error: Server Disconnected while sending payload'),
          isTrue,
        );
  ...
  group(
    'CHALLENGER STRESS: ActiveServerManager Connection Error & Headless L10n',
    () {
      test(
        'reportConnectionError with active, inactive, custom, and localized messages',
        () async {
      ...
      test(
        'Session check unreachable and serverError statuses update error state in EN & RU',
        () async {
          final s1 = ServerProfile(
            id: 'srv_1',
            displayName: 'Alpha Server',
          ...
          await secretStore.saveToken(
            serverId: 'srv_1',
            token: 'existing_token',
          );

          final gateway = _MockSessionCheckAuthGateway();
          final auth = AuthService(secretStore: secretStore, gateway: gateway);
  ...
  group(
    'CHALLENGER STRESS: ServerSetupScreen Validation & Error Handling Widget Tests',
    () {
      testWidgets(
        'Validates each field individually and shows localized snackbars in EN',
        (tester) async {
      ...
      testWidgets(
        'Validates each field individually and shows localized snackbars in RU',
        (tester) async {
          tester.view.physicalSize = const Size(1080, 2200);
          tester.view.devicePixelRatio = 1.0;
          addTearDown(tester.view.resetPhysicalSize);
      ...
      testWidgets(
        'Handles AuthException and displays localized error snackbar in RU',
        (tester) async {
          tester.view.physicalSize = const Size(1080, 2200);
          tester.view.devicePixelRatio = 1.0;
          addTearDown(tester.view.resetPhysicalSize);
          ...
          expect(
            find.text(
              'Ошибка аутентификации: Incorrect password or user not found',
            ),
            findsOneWidget,
          );
        },
      ...
      testWidgets(
        'Handles AuthException and displays localized error snackbar in EN',
        (tester) async {
          tester.view.physicalSize = const Size(1080, 2200);
          tester.view.devicePixelRatio = 1.0;
          addTearDown(tester.view.resetPhysicalSize);
          ...
          expect(
            find.text(
              'Authentication failed: Incorrect password or user not found',
            ),
            findsOneWidget,
          );
        },
      ...
      testWidgets(
        'Handles Generic Exception with fallback localized snackbar in RU',
        (tester) async {
          tester.view.physicalSize = const Size(1080, 2200);
          tester.view.devicePixelRatio = 1.0;
          addTearDown(tester.view.resetPhysicalSize);
      ...
      testWidgets(
        'Handles Generic Exception with fallback localized snackbar in EN',
        (tester) async {
          tester.view.physicalSize = const Size(1080, 2200);
          tester.view.devicePixelRatio = 1.0;
          addTearDown(tester.view.resetPhysicalSize);
  ...
  group(
    'CHALLENGER STRESS: AuthCard Tabs, Password Visibility & Forgot Password Dialog',
    () {
      testWidgets(
        'Toggles AuthMode tabs and updates buttons and footnotes in EN & RU',
        (tester) async {
          ...
          expect(
            find.text('Нет аккаунта? Перейти к регистрации.'),
            findsOneWidget,
          );
          ...
          expect(
            find.text('Уже есть аккаунт? Перейти ко входу.'),
            findsOneWidget,
          );
      ...
      testWidgets(
        'Forgot password flow with missing server URL shows localized snackbar in RU',
        (tester) async {
          final userCtrl = TextEditingController();
          final passCtrl = TextEditingController();
          final ruL10n = lookupAppLocalizations(const Locale('ru'));
      ...
      testWidgets(
        'Forgot password flow with missing server URL shows localized snackbar in EN',
        (tester) async {
          final userCtrl = TextEditingController();
          final passCtrl = TextEditingController();
          final enL10n = lookupAppLocalizations(const Locale('en'));
      ...
      testWidgets(
        'Forgot password dialog step 1 validation and error handling in RU',
        (tester) async {
          tester.view.physicalSize = const Size(1080, 2200);
          tester.view.devicePixelRatio = 1.0;
          addTearDown(tester.view.resetPhysicalSize);
      ...
      testWidgets(
        'Forgot password dialog step 1 validation and error handling in EN',
        (tester) async {
          tester.view.physicalSize = const Size(1080, 2200);
          tester.view.devicePixelRatio = 1.0;
          addTearDown(tester.view.resetPhysicalSize);
      ...
      testWidgets(
        'Biometric login button displays properly when enabled and triggers callback',
        (tester) async {
          final userCtrl = TextEditingController();
          final passCtrl = TextEditingController();
