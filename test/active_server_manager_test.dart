...
void main() {
  ...
  group('ActiveServerManager Localization & Error Handling', () {
    test('reportConnectionError sets localized default message in English and Russian', () async {
      final s1 = ServerProfile(
        id: 's1',
      ...
      manager.reportConnectionError(serverId: 's1', message: 'Custom network timeout');
      expect(manager.connectionErrorMessage, 'Custom network timeout');

      // Test reporting error for inactive server is ignored
      manager.reportConnectionError(serverId: 'other-server', message: 'Ignored error');
      expect(manager.connectionErrorMessage, 'Custom network timeout');
    });

    test('lifecycle methods accept optional l10n parameter and execute cleanly', () async {
      final s1 = ServerProfile(
        id: 's1',
        displayName: 'Server 1',
        baseUrl: 'https://s1',
