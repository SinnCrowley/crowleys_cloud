...
void main() {
  ...
  group('CacheService Adversarial: HTTP 304 & ETag Nuances', () {
    test(
      '304 Not Modified when cached file missing on disk returns null without crash',
      () async {
        // 1. Initial 200 OK
    ...
    test(
      '304 Not Modified with refreshed ETag header updates manifest etag',
      () async {
    ...
    test(
      'Handles weak ETags (W/"...") and unquoted ETags seamlessly',
      () async {
        await CacheService.instance.getRemoteThumbnail(
          serverId: 'srv1',
  ...
  group('CacheService Adversarial: Offline Fallback & Network Error Handling', () {
    test(
      'Offline fallback returns cached disk thumbnail when network fetch fails (returns null)',
      () async {
    ...
    test(
      'Non-cached item returning null from fetch returns null without throwing',
      () async {
        final result = await CacheService.instance.getRemoteThumbnail(
          serverId: 'srv1',
  ...
  group('CacheService Adversarial: Manifest Corruption & Recovery Harness', () {
    test(
      'Recovers gracefully from corrupted JSON text in manifest.json',
      () async {
        final manifest = File(
          p.join(supportDir.path, 'cache', 'manifest.json'),
        );
    ...
    test(
      'Recovers gracefully from malformed manifest structure (array root, null fields)',
      () async {
        final manifest = File(
          p.join(supportDir.path, 'cache', 'manifest.json'),
        );
  ...
  group('CacheService Adversarial: Concurrency & Eviction Stress', () {
    test(
      'Concurrent requests for same thumbnail coalesce into a single in-flight fetch',
      () async {
        var fetchCount = 0;
    ...
    test(
      'LRU eviction handles rapid successive writes exceeding max bytes limit',
      () async {
        ...
        for (int i = 0; i < 20; i++) {
          await CacheService.instance.getRemoteThumbnail(
            ...
            cacheKey: 'item_$i',
            fetch: (etag) async =>
                ThumbnailFetchResult.bytes(Uint8List(10), etag: '"etag-$i"'),
          );
